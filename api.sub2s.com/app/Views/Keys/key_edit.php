...
<div class="row pb-5 justify-content-center">
    ...
    <label for="expired_date" class="form-label fw-bold">Expired Date <?= !$key->expired_date ? '<span class="badge bg-warning text-dark">Not started yet</span>' : '' ?></label>
    <div class="input-group mb-2">
        <span class="input-group-text bg-dark text-white"><i class="bi bi-calendar-event"></i></span>
        <input type="text" name="expired_date" id="expired_date" class="form-control" placeholder="YYYY-MM-DD HH:mm:ss" value="<?= old('expired_date') ?: $key->expired_date ?>">
    </div>

    <div class="d-flex flex-wrap gap-2 mb-2">
        <div class="btn-group btn-group-sm">
            <button type="button" class="btn btn-outline-primary" onclick="adjustTime(1, 'hour')">+1h</button>
            <button type="button" class="btn btn-outline-primary" onclick="adjustTime(-1, 'hour')">-1h</button>
        </div>
        <div class="btn-group btn-group-sm">
            <button type="button" class="btn btn-outline-success" onclick="adjustTime(1, 'day')">+1d</button>
            <button type="button" class="btn btn-outline-success" onclick="adjustTime(-1, 'day')">-1d</button>
        </div>
        <div class="btn-group btn-group-sm">
            <button type="button" class="btn btn-outline-info" onclick="adjustTime(1, 'month')">+1m</button>
            <button type="button" class="btn btn-outline-info" onclick="adjustTime(-1, 'month')">-1m</button>
        </div>
        <button type="button" class="btn btn-sm btn-dark" onclick="resetToNow()">Set Now</button>
    </div>
    
    <small class="text-muted">Định dạng chuẩn: <code>YYYY-MM-DD HH:mm:ss</code></small>
</div>
...
<script>
    $(document).ready(function() {
        ...
        // Nếu không phải admin thì khóa các trường quan trọng
        if (level != 1) {
            $("#registrator, #expired_date, #devices, .btn-group button, button[onclick^='resetToNow']").attr('disabled', true);
        }

        // Bật nút Update khi có bất kỳ thay đổi nào
        $("input, select, textarea").on('input change', function() {
            $(".btnUpdate").attr('disabled', false).removeClass('btn-outline-info').addClass('btn-info text-white');
        });
    });

    // Hàm điều chỉnh thời gian
    function adjustTime(value, unit) {
        let input = $('#expired_date');
        let currentStr = input.val();
        
        // Nếu ô đang trống hoặc null, lấy thời gian hiện tại làm mốc
        let date = (currentStr && currentStr !== '0000-00-00 00:00:00') ? new Date(currentStr) : new Date();

        if (isNaN(date.getTime())) {
            alert("Định dạng ngày hiện tại không hợp lệ!");
            return;
        }
        ...
        // Cập nhật lại ô input theo định dạng YYYY-MM-DD HH:mm:ss
        input.val(formatDate(date)).trigger('change');
    }

    // Hàm đặt về thời gian hiện tại
    function resetToNow() {
        $('#expired_date').val(formatDate(new Date())).trigger('change');
    }

    // Hàm format ngày thành chuỗi SQL chuẩn
    ...
    // Logic cũ cho devices
    var total = "<?= $key_info->total ?>";
    $("#max_devices").on('change input', function() {
