...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `Get link - ${slug}`,
    description: "Liên kết được rút gọn và bảo vệ qua hệ thống Sub2S.",
  };
}
...
export default async function LinkPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [freeLinkEnabledRaw, defaultPriceRaw, adLayersRaw, interstitialAdUrl] = await Promise.all([
  ...
  // Lấy tất cả URL quảng cáo đang bật
